...
// exports.member_editById = async (req, res, next) => {
//   try {
...
exports.members_deleteById = async (req, res, next) => {
  try {
    const { userId } = req.params
    const result = await query(`DELETE FROM member WHERE user_id = ${userId}`)
    res.status(200)
    res.json({ result })
  }
  catch(error) {
